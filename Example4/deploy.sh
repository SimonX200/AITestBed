...
echo "=== Step 2: Compile sessionManager.ts to dist/bundle.js ==="
mkdir -p dist
npx esbuild sessionManager.ts --bundle --outfile=dist/bundle.js --platform=node --target=node20
...
docker ps --filter "name=session-manager-container" --format "table {{.ID}}\t{{.Names}}\t{{.Status}}"