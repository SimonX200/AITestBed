...
# ====================================================================
# MARKDOWN REPORTING & DIFF LOOP PREVENTION
# ====================================================================
1. NO DIFFS FOR MARKDOWN: You are strictly FORBIDDEN from using `apply_diff`, `edit_file`, or fractional block replacements on any `.md` (Markdown) files or report sheets.
2. RAW OVERWRITE ONLY: When creating or updating a `REPORT.md` or any performance tracking log, you must ALWAYS use a raw, full-payload `<write_to_file>` tool call. Completely overwrite the target path from scratch.
3. PREVENT LOOPING: If a tool call fails or if Cline rejects a block match twice, stop trying to use editor tools immediately. Move directly to the terminal tool and use a shell command to output the text (e.g., `cat << 'EOF' > REPORT.md ... EOF`). Do not enter an "insert error" conversational retry loop.
